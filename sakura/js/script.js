...
jQuery(function () {
  if (!navigator.userAgent.match(/iPhone|iPad|Android.+Mobile/)) {
    // スクロール値取得
    jQuery(window).scroll(function () {
      var scroll = jQuery(window).scrollTop(); // 拡大縮小スケール、ぼかし
      jQuery("#pp-scale").css({
        transform: "scale(" + (100 + scroll / 10) / 100 + ")",
        filter: "blur(" + scroll / 100 + "px)",
      });
      // スクロール量調整
      jQuery("#about__bg-s").css(
        "background-position",
        "left top " + parseInt(-scroll / 100) + "px"
      );
      jQuery("#about__bg-m").css(
        "background-position",
        "left top " + parseInt(-scroll / 10) + "px"
      );
      jQuery("#about__bg-l").css(
        "background-position",
        "left top " + parseInt(-scroll / 2) + "px"
      );
    });
  }
});
  ...
  particlesJS("wrapper", {
    particles: {
      ...
      shape: {
        ...
        image: {
          src:
            "https://kazuki-portfolio.work/portfolio/wp-content/themes/sakura/img/sakura.png",
          width: 120,
          height: 120,
        },
      },
